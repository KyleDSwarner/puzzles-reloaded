...
static float *game_colours(frontend *fe, int *ncolours)
{
    ...
    ret[COL_SYMBOL * 3 + 0] = ret[COL_BACKGROUND * 3 + 0];
    ret[COL_SYMBOL * 3 + 1] = ret[COL_BACKGROUND * 3 + 1];
    ret[COL_SYMBOL * 3 + 2] = ret[COL_BACKGROUND * 3 + 2];
                      ...
                      int x, int y, unsigned long flags)
{
    ...
    if (which == EMPTY && !(flags & DS_SET)) {
        ...
        fg = (flags & DS_ERROR) ? COL_ERROR :
             (flags & DS_CURSOR) ? COL_TEXT : COL_SYMBOL;
